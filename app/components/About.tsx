        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <ScrollReveal variant="fade-up">
              ...
              <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white uppercase mt-4">
                Take risk.
                <div>
              ...
              <p className="text-base sm:text-lg text-gray-300 font-normal leading-relaxed mt-4">
              ...
              {pillars.map((pillar, idx) => (
                ...
                >
                  <div className="card-glass p-5 rounded-xl group cursor-default h-full transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_12px_30px_rgba(239,35,60,0.18)] hover:border-red-500/40">
                    ...
                    <h3 className="text-sm font-bold text-white mb-1 group-hover:text-[#ef233c] transition-colors">
                      {pillar.title}
                    </h3>
          ...
          <ScrollReveal variant="fade-left" delay={200} className="lg:col-span-5 w-full">
            <div className="card-glass rounded-2xl p-7 relative overflow-hidden border border-white/10 shadow-2xl hover:border-red-500/30 transition-all duration-500">
              <div className="flex items-center justify-between pb-5 border-b border-white/10">
                <div>
                  <span className="text-[11px] font-mono uppercase tracking-widest text-[#ef233c]">
                    BIOMEDICAL_DOSSIER
                  </span>
                  <h3 className="text-xl font-bold text-white tracking-tight">Sridhar SK</h3>
                </div>
              ...
              <div className="space-y-4 pt-5 font-mono text-xs">
                {dossier.map((item) => (
                  <div
                    key={item.label}
                    className="flex justify-between items-center py-2 border-b border-white/10 hover:bg-white/[0.04] px-2 rounded transition-all duration-200"
                  >
                    <span className="text-gray-300 font-medium">{item.label}</span>
                    <span
                      className={`text-right ${
                        item.highlight ? "text-red-400 font-bold text-sm" : "text-white font-semibold"
                      }`}
                    >
                      {item.value}
                    </span>
              ...
              <div className="mt-6 pt-5 border-t border-white/10 bg-white/[0.02] -mx-7 -mb-7 p-5 flex items-center justify-between">
                <span className="text-xs text-gray-400">Hometown</span>
